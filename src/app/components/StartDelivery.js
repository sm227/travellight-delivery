...
export function StartDelivery({ isWorking, onStartWork, onNext }) {
  ...
  return (
    <div className="p-4 flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">배달 시작</h2>
      ...
      {!isWorking ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 text-center">
          ...
          <h3 className="text-lg font-medium mb-2 text-gray-900 dark:text-gray-100">운행을 시작해볼까요?</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6">운행을 시작하면 근처의 배달 요청이 표시됩니다.</p>
          <button 
            className="w-full py-4 bg-blue-600 dark:bg-blue-700 text-white rounded-lg font-medium hover:bg-blue-700 dark:hover:bg-blue-600"
            onClick={onStartWork}
          >
            운행 시작하기
          </button>
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-medium text-lg text-gray-900 dark:text-gray-100">가능한 배달</h3>
              <button 
                onClick={fetchDeliveries} 
                disabled={loading}
                className="text-blue-600 dark:text-blue-400 text-sm font-medium hover:text-blue-700 dark:hover:text-blue-300 disabled:opacity-50"
              >
                {loading ? '검색 중...' : '새로고침'}
              </button>
            </div>
            ...
            {loading && availableDeliveries.length === 0 ? (
              <div className="text-center py-10">
                <div className="w-10 h-10 border-t-2 border-b-2 border-blue-600 dark:border-blue-400 rounded-full animate-spin mx-auto mb-4"></div>
                <p className="text-gray-500 dark:text-gray-400">주변 배달 요청을 검색 중입니다...</p>
              </div>
              ...
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-2">아직 가능한 배달이 없습니다</p>
                <p className="text-sm text-gray-400 dark:text-gray-500">잠시 후 다시 시도해 주세요</p>
              </div>
            ) : (
                ...
                {availableDeliveries.map(delivery => (
                  ...
                  >
                    <div className="flex justify-between">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 font-bold">{delivery.price.toLocaleString()}원</span>
                    </div>
          ...
          {selectedDelivery && (
            <div className="sticky bottom-0 bg-white dark:bg-gray-800 p-4 border-t border-gray-200 dark:border-gray-600 rounded-t-lg shadow-lg">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium text-gray-900 dark:text-gray-100">배달 수락</h3>
                <span className="text-blue-600 dark:text-blue-400 font-bold">{selectedDelivery.price.toLocaleString()}원</span>
              </div>
              <button
                className="w-full py-4 bg-blue-600 dark:bg-blue-700 text-white rounded-lg font-medium hover:bg-blue-700 dark:hover:bg-blue-600"
